...
def test_embedded_payload_identity_collects_embedded_analysis_for_unmatched_extension(tmp_path, monkeypatch):
    target = tmp_path / "notes.txt"
    target.write_bytes(b"plain text")
    bag = FactBag()

    decision = DetectionScheduler(_config([
        {"name": "embedded_payload_identity", "enabled": True, "carrier_exts": [".jpg"], "ambiguous_resource_exts": [".bin"]},
...
def test_scoring_fact_requirement_collects_embedded_archive_for_matched_extension(tmp_path):
    ...
    decision = DetectionScheduler(_config([
        {"name": "embedded_payload_identity", "enabled": True, "carrier_exts": [".jpg"], "ambiguous_resource_exts": []},
    ])).evaluate(bag, FactProvider(str(target)))
...
def test_empty_scoring_fact_requirements_keep_required_facts_unconditional(tmp_path, monkeypatch):
    ...
    target.write_bytes(b"7z\xbc\xaf\x27\x1c")
...
def test_embedded_payload_identity_ignores_exe_loose_scan_after_collecting_inputs(tmp_path, monkeypatch):
    ...
    def counting_fill_fact(self, fact_bag, fact_name):
        if fact_name in {"file.magic_bytes", "embedded_archive.analysis", "pe.overlay_structure"}:
    ...
    assert bag.has("pe.overlay_structure")
