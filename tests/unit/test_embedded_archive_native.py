...
def test_native_carrier_archive_failure_is_reported(tmp_path, monkeypatch):
    target = tmp_path / "image.png"
    ...
    with pytest.raises(RuntimeError, match="native scan unavailable"):
        embedded_archive._find_after_carrier(
            str(target),
            ".png",
            target.stat().st_size,
...
def test_native_carrier_tail_marker_without_magic_skips_full_scan(tmp_path):
    target = tmp_path / "image.jpg"
...
def test_native_carrier_tail_uses_earlier_marker_when_payload_contains_marker(tmp_path):
    target = tmp_path / "image.jpg"
...
def test_native_carrier_tail_failure_is_reported(tmp_path, monkeypatch):
    target = tmp_path / "image.jpg"
    ...
    with pytest.raises(RuntimeError, match="native scan unavailable"):
        embedded_archive._find_tail_after_markers_layered(
            str(target),
            (b"\xff\xd9",),
            target.stat().st_size,
...
def test_native_carrier_tail_no_hit_is_trusted(tmp_path, monkeypatch):
    target = tmp_path / "image.jpg"
    target.write_bytes(b"\xff\xd8image\xff\xd9payload")
...
def test_native_carrier_tail_ignores_disable_environment(tmp_path, monkeypatch):
    target = tmp_path / "image.jpg"
    target.write_bytes(b"\xff\xd8image\xff\xd9" + b"7z\xbc\xaf\x27\x1c" + b"payload")
    ...
    with pytest.raises(AssertionError, match="native scan should be disabled"):
        embedded_archive._find_tail_after_markers_layered(
            str(target),
            (b"\xff\xd9",),
            target.stat().st_size,
...
def test_native_loose_magic_scan_no_hit_is_trusted(tmp_path, monkeypatch):
    target = tmp_path / "payload.bin"
    target.write_bytes(b"prefix")
...
def test_native_loose_magic_scan_failure_is_reported(tmp_path, monkeypatch):
    target = tmp_path / "payload.bin"
    ...
    with pytest.raises(RuntimeError, match="native scan unavailable"):
        embedded_archive._stream_find_tail_magics_anywhere(
            str(target),
            min_offset=0,
            max_hits=2,
        )
...
def test_native_loose_magic_scan_ignores_disable_environment(tmp_path, monkeypatch):
    target = tmp_path / "payload.bin"
