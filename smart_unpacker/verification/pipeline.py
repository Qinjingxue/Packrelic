...
def _merge_coverage_sources(sources: list[dict]) -> ArchiveCoverageSummary:
    ...
    matched_files = max(_as_int(item.get("matched_files")) for item in sources)
    complete_files = max(_as_int(item.get("complete_files")) for item in sources)
    partial_files = max(_as_int(item.get("partial_files")) for item in sources)
    failed_files = max(_as_int(item.get("failed_files")) for item in sources)
    missing_files = max(_as_int(item.get("missing_files")) for item in sources)
    matched_bytes = max(_as_int(item.get("matched_bytes")) for item in sources)
    complete_bytes = max(_as_int(item.get("complete_bytes")) for item in sources)
