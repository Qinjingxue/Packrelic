...
from smart_unpacker_native import gzip_deflate_member_resync_repair as _native_gzip_member_resync
    ...
    def repair(self, job: RepairJob, diagnosis: RepairDiagnosis, workspace: str, config: dict) -> RepairResult:
        ...
        if status == "partial" and selected_path:
            return RepairResult(
                ...
                format="gzip",
                repaired_input={"kind": "file", "path": selected_path, "format_hint": "gzip"},
                ...
                diagnosis={**diagnosis.as_dict(), "native_gzip_deflate_member_resync": result},
                message=str(result.get("message") or "native gzip member resync produced a candidate"),
            )
        return RepairResult(
            ...
            module_name=self.spec.name,
            diagnosis={**diagnosis.as_dict(), "native_gzip_deflate_member_resync": result},
            message=str(result.get("message") or "native gzip member resync did not produce a candidate"),
        )
