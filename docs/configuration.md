...
| `archive_identity` | 汇总通用起始魔数和嵌入载荷证据；7z/RAR 起始结构由专用结构处理器负责。 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `magic_start_score` | `int` | 文件开头魔数命中时的分数。 |
...
| `identity_scan_exts` | `list[str]` | 需要做身份扫描的扩展名。 |
...
历史 `magic_bytes`、`embedded_archive` scoring 规则已移出 active 规则包；当前主流水线通过 `archive_identity` 消费 `file.magic_bytes` 和 `embedded_archive.analysis`，通过 `seven_zip_structure_identity`、`rar_structure_identity` 消费 7z/RAR 起始结构 fact。
...
- 想提高召回率，优先调整 `extension`、结构类 identity 规则、`archive_identity` 和阈值。
