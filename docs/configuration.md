...
| `deep.max_entries` | `int` | deep 模块单次最多扫描/恢复的条目数量。 |
| `deep.max_seconds_per_module` | `int` / `float` | deep 模块单次预算秒数，`0` 表示不限制。 |
| `deep.max_input_size_mb` | `int` / `float` | deep 模块可处理的输入大小上限，`0` 表示不限制。 |
| `deep.max_output_size_mb` | `int` / `float` | deep 模块写出的候选归档大小上限，`0` 表示不限制。 |
| `deep.max_entry_uncompressed_mb` | `int` / `float` | deep 模块验证单个压缩条目时允许解出的最大大小，`0` 表示不限制。 |
| `deep.verify_candidates` | `bool` | deep 模块是否应在返回前验证候选。 |
| `auto_deep.enabled` | `bool` | targeted/safe 无法产生可接受改进时，是否允许调度器自动放行少量 deep 候选。 |
| `auto_deep.require_verification_repair` | `bool` | 自动 deep 是否必须由 verification 的 repair 决策触发。 |
| `auto_deep.max_modules` | `int` | 自动 deep 单轮最多尝试多少个 deep 模块。 |
| `auto_deep.max_candidates_per_module` | `int` | 自动 deep 每个模块最多生成多少个候选。 |
| `auto_deep.max_input_size_mb` | `int` / `float` | 输入超过该大小时不自动升级 deep，`0` 表示不限制。 |
| `beam.enabled` | `bool` | 是否使用 beam 评估多个 patch plan。关闭后 repair 不会接管 verification repair 决策。 |
| `beam.beam_width` | `int` | 每轮保留的候选状态数量。 |
| `beam.max_candidates_per_state` | `int` | 单个状态最多展开多少个 patch plan。 |
| `beam.max_analyze_candidates` | `int` | 每轮进入 analysis 的候选上限。 |
| `beam.max_assess_candidates` | `int` | 每轮进入 verification 比较的候选上限。 |
| `beam.max_rounds` | `int` | 单个 repair 循环最多展开多少轮 beam。 |
| `beam.min_improvement` | `int` / `float` | 候选完整度必须超过当前结果的最小幅度。 |

`repair` 只响应 verification 的 `repair` 决策：analysis 的中等置信度和 extraction 失败都会先产出 extraction result，再交给 verification 计算完整度与进度。repair 生成的 patch plan 通过 beam 重新解压、重新 verification，并且只有比当前结果更完整时才会继续进入下一轮；没有提升时循环主动停止。

旧键 `repair.trigger_on_medium_confidence`、`repair.trigger_on_extraction_failure` 和 `repair.thresholds` 已移除，配置中出现这些键会直接报错。新增高成本模块时应声明 `stage="deep"`，并根据行为设置 `safe`、`partial`、`lossy` 元数据，让调度器用上面的安全门过滤。
