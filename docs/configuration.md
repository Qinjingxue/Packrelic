...
| `archive_identity` | 汇总魔数、嵌入载荷和 ZIP 结构证据。 |
...
| `sfx_hint_score` | `int` | SFX 线索命中时的分数。 |
...
历史 `magic_bytes`、`embedded_archive` scoring 规则已移出 active 规则包；当前主流水线通过 `archive_identity` 消费 `file.magic_bytes` 和 `embedded_archive.analysis`。
...
- 想提高召回率，优先调整 `extension`、`archive_identity` 和阈值。
