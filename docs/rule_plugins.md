...
```python
from smart_unpacker.detection.pipeline.rules.base import RuleBase, RuleEffect
...
- `config_schema`：允许出现在 `smart_unpacker_config.json` 里的规则配置字段。
...
完整事实名以 `smart_unpacker/detection/pipeline/facts/schema.py`、各插件注册结果和 `python sunpack_cli.py config validate --json` 的 `registered_facts` 为准。
