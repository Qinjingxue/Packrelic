...
python -m maturin build --manifest-path native\smart_unpacker_native\Cargo.toml --release
```

Install the generated wheel:

```powershell
$wheel = Get-ChildItem native\smart_unpacker_native\target\wheels\smart_unpacker_native-*.whl | Select-Object -First 1 -ExpandProperty FullName
...
```powershell
python -c "import smart_unpacker_native; print(smart_unpacker_native.native_available(), smart_unpacker_native.scanner_version())"
```
