...
using smart_unpacker::sevenzip::ArchiveOperationRequest;
using smart_unpacker::sevenzip::ArchiveOperationResult;
using smart_unpacker::sevenzip::ExtractInputRange;
using smart_unpacker::sevenzip::PasswordTestStatus;
...
ArchiveOperationRequest to_request(const Sup7zOperationRequest& request) {
    ...
    operation.part_paths = smart_unpacker::sevenzip::capi::collect_part_paths(
...
void copy_result(const ArchiveOperationResult& source, Sup7zOperationResult* destination) {
    ...
    smart_unpacker::sevenzip::capi::copy_wide(destination->archive_type, 64, source.archive_type);
    smart_unpacker::sevenzip::capi::copy_message(destination->message, 512, source.message);
}
...
SUP7Z_API int sup7z_run_operation(
    ...
    const ArchiveOperationResult operation_result = smart_unpacker::sevenzip::run_archive_operation(to_request(*request));
