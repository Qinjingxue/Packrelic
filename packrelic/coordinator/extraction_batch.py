...
from packrelic.coordinator.repair_beam import RepairBeamCandidate, RepairBeamLoop, RepairBeamState
from packrelic.coordinator.repair_loop import RepairLoopLimits, RepairLoopState, terminal_failure_reason
from packrelic.coordinator.repair_stage import ArchiveRepairStage
from packrelic.coordinator.resource_preflight import ResourcePreflightInspector
from packrelic.coordinator.scheduling import (
...
from packrelic.verification.result import DECISION_ACCEPT, DECISION_ACCEPT_PARTIAL, DECISION_REPAIR, DECISION_RETRY_EXTRACT
from packrelic.support.path_keys import absolute_path_key
...
class ExtractionBatchRunner:
    ...
    def _promote_beam_output(self, result: ExtractionResult, temp_dir: str, out_dir: str) -> ExtractionResult:
        ...
        manifest = Path(out_dir) / ".packrelic" / "extraction_manifest.json"
...
def _retarget_result_output(result: ExtractionResult, old_dir: str, new_dir: str) -> None:
    ...
    try:
        ...
        candidate = new / ".packrelic" / "extraction_manifest.json"
...
def _write_recovery_report(
    ...
    *,
    ...
    target = Path(out_dir) / ".packrelic" / "recovery_report.json"
    try:
